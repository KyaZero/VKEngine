...
vke::Logger* vke::Logger::s_Instance = nullptr;

vke::Logger::Logger() : m_Buffer(nullptr), m_Queue(), m_Level((char)Level::All), m_Thread(nullptr), m_ShouldLogToFile(true), m_ShouldPrint(true), m_MultiThreaded(false)
{
...
void vke::Logger::Create(bool multiThreaded)
{
...
void vke::Logger::Log(Level level, const char* file, u32 line, const char* function, const char* format, ...)
{
	if (!s_Instance)
		return;
...
#ifdef _WIN32
auto vke::Logger::GetLevelString(Level level)
{
	switch (level)
	{
...
#else
std::string vke::Logger::GetLevelString(Level level)
{
	switch (level)
	{
	case Logger::Level::Verbose:
...
void vke::Logger::LogToFile(const std::string& msg)
{
...
void vke::Logger::Update(Logger* instance, std::chrono::duration<double, std::milli> interval)
{
...
void vke::Logger::Print(Logger* instance, const LogEntry& entry)
{
#ifdef _WIN32
...
void vke::Logger::SetLevel(Level level)
{
	Get()->m_Level = (char)level;
}

void vke::Logger::SetPrint(bool shouldPrint)
{
	Get()->m_ShouldPrint = shouldPrint;
}

void vke::Logger::SetShouldLogToFile(bool shouldLogToFile)
{
